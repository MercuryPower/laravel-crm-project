        ...
        <div class="w-2/4 mx-auto sm:px-6 lg:px-8">
            <form action="{{route('roles.update', $role->id)}}"  method="post">
                ...
                <div class="mt-6 max-w-md">
                    ...
                    <input value="{{$role ->name}}" type="text"   required
                    ...
                    <button type="submit"  class="my-3 bg-violet-600 hover:bg-violet-100 text-white hover:border-violet-600 border
                        hover:text-Аviolet-600 px-4 py-2 rounded">
                        Добавить
                    </button>
                </div>
                @foreach($permissions as $permission)
                    <div class="btn-group  p-6 px-4 py-2" role="group" aria-label="Basic checkbox toggle button group">
                        <input type="checkbox" class="btn-check" value="{{$permission ->id}}" @if($role->hasPermissionTo($permission->name)) checked @endif   name="permissions[]" autocomplete="off">
